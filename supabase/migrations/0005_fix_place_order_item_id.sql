-- Fix the line-item insert in the live order RPC.
...
begin
  ...
  for v_item in select * from jsonb_to_recordset(p_items) as x(item_id uuid, qty int) loop
    ...
    if v_stock < v_item.qty then
      raise exception 'Insufficient stock for item % (requested %, available %)', v_item.item_id, v_item.qty, v_stock;
    end if;
  ...
  insert into public.orders (
    order_number, customer_id, status, order_type, customer_name, customer_phone,
    customer_email, address, note, subtotal_kyat, delivery_fee, total_kyat, payment_method
  ) values (
    v_order_number, v_customer_id, 'pending', p_order_type, v_customer_name, p_customer_phone,
    p_customer_email, p_address, p_note, v_subtotal, p_delivery_fee, v_total, 'cod'
  ...
  return jsonb_build_object('order_id', v_order_id, 'order_number', v_order_number, 'total_kyat', v_total);
end $$;
